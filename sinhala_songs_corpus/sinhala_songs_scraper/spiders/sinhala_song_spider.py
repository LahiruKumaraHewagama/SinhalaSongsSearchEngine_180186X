...
class SinhlaSongSpider(scrapy.Spider):
    ...
    start_urls = ["https://sinhalasongbook.com/all-sinhala-song-lyrics-and-chords/?_page=" + str(x) for x in range(1,26)]    
    def parse(self, response):     
        ...
        for href in response.xpath("//main[contains(@id, 'genesis-content')]//div[contains(@class, 'entry-content')]//div[contains(@class, 'pt-cv-wrapper')]//h4[contains(@class, 'pt-cv-title')]/a/@href"):
            href =  href.extract()
            list_count.append(href.split('https://www.sinhalasongbook.com/')[1][0])
            if(list_count.count(href.split('https://www.sinhalasongbook.com/')[1][0])<=4):
                # print(href.split('https://www.sinhalasongbook.com/')[1][0])
    ...
    def parse_lyrics_from_href(self,response):
        ...
        lyricist = song_page.xpath('//div[@class="su-row"]//span[@class="lyrics"]//a/text()').extract()
        lyricist_data = (','.join(lyricist))
